...
Note that there is a known bug with opam 1.2 that will prevent Owl from running once it's installed.  One solution is to upgrade to one of the opam 2.0.0 beta versions--at least beta4.  Another solution is to install Owl with opam 1.2, but after installation, move the file dllowl_stubs.so from ~/.opam/.../lib/stubslibs to ~/.opam/.../lib/stublibs.  (The "..." should be filled in with the name of your current OCaml compiler switch, e.g. "4.06.0".)
