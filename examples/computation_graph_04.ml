...
let visualise_mnist () =
  ...
  let xt = G.var_arr ~name:"xt" [|100;32;32;3|] |> A.pack_arr in
  let yt = G.var_arr ~name:"yt" [|100;10|] |> A.pack_arr in
  ...
  let s0 = G.to_dot [loss |> A.unpack_elt |> G.elt_to_node] in
...
let visualise_lstm () =
  ...
  let xt = G.var_arr ~name:"xt" [|10;50|] |> A.pack_arr in
  let yt = G.var_arr ~name:"yt" [|10;26|] |> A.pack_arr in
  let yt', _ = Graph.(init network; forward network xt) in
  let loss = A.(Maths.((cross_entropy yt yt') / (pack_flt (Mat.row_num yt |> float_of_int)))) in
  let _, adj0 = Graph.(backward network loss) in
