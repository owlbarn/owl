...
= struct
  ...
  module Maths = struct
    ...
    and _lyapunov_backward_a a aa ap = 
      let s = lyapunov (transpose a) (neg aa) in
      (pack_flt 2.) * s *@ ap

    and _lyapunov_backward_q a aa = neg (lyapunov (transpose a) (neg aa)) 
    ...
    and discrete_lyapunov ?(solver=`default) a q =
      ...
      let df_daq cp ap at _qp qt =
        ...
        + (discrete_lyapunov ap qt) in
      let r_d_d a q = Discrete_Lyapunov_D_D (a, q) in
      let r_d_c a q = Discrete_Lyapunov_D_C (a, q) in
      let r_c_d a q = Discrete_Lyapunov_C_D (a, q) in
      op_d_d_d a q ff fd df_da df_dq df_daq r_d_d r_d_c r_c_d

    and _discrete_lyapunov_backward_a a aa ap = 
      let s =  discrete_lyapunov (transpose a) aa in
      (pack_flt 2.) * s *@ a *@ ap

    and _discrete_lyapunov_backward_q a aa = discrete_lyapunov (transpose a) aa

    and _discrete_lyapunov_backward_aq a aa ap =
      let s = discrete_lyapunov (transpose a) aa in
      (pack_flt 2.) * s *@ a *@ ap, s
