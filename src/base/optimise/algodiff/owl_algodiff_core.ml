module Make (Types : Owl_algodiff_types_sig.Sig) = struct
  ...
  and op_s_m a ff fd df r =
    ...
    | DR (ap, _, _, _, ai, _) ->
      ...
      Array.map2
        (fun cp aa -> DR (cp, aa, r (a, cp_arr, aa_arr), ref 0, ai, tracker))
        cp_arr
        aa_arr
    | ap -> ff ap
  ...
  and op_p_s a b ff fd df_da df_db df_dab r_d_d r_d_c r_c_d =
    match a, b with
    | F _ap, DF (bp, bt, bi) ->
      let cp = fd a bp in
