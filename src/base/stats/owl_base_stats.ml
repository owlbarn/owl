...
let sort ?(inc = true) x =
  ...
  Array.sort (fun a b -> if a < b then -c else if a > b then c else 0) y;
  y
...
let hist_weighted_uniform n w x =
  if Array.(length x <> length w)
  then failwith "Data and weights must have the same length.";
  let _bmin, _bmax, _db, bins, get_bin = setup_uniform_binning n x in
  let c = Array.make n 0 in
...
let setup_nonuniform_binning bins =
  ...
  let get_bin y =
    ...
    if i = n && fcmp y bins.(n) = 0 then i - 1 else i
  in
  n, get_bin
...
let init_sorted bins x =
  ...
  let i, j = if fcmp bins.(0) x.(0) < 1 then bs x.(0) bins, 0 else 0, 1 + bs bins.(0) x in
  assert (0 <= i && i <= n);
  assert (0 <= j && j < m);
  n, m, ref i, ref j
...
let hist_to_string { bins; counts; weighted_counts; normalised_counts; density } =
  ...
  let n = Array.length bins - 1 in
  let w =
    match weighted_counts with
    | None    -> ""
    | Some wc ->
      let tot = Array.fold_left ( +. ) 0. wc in
      Printf.sprintf "; tot. weight: %g" tot
  in
  let norm =
    match normalised_counts with
    | None   -> ""
    | Some _ -> "; normalised"
  in
  let pdf =
    ...
    | Some _ -> "; density"
  in
  Printf.sprintf "[ N: %i; N_bins: %i%s%s%s ]" n_counts n w norm pdf


(* now the external api *)

let histogram (bins : [ `N of int | `Bins of float array ]) ?weights x =
  let (bins, counts), weighted_counts =
    match bins, weights with
    | `N n, None      -> hist_uniform n x, None
    | `Bins b, None   -> hist_nonuniform b x, None
    | `N n, Some w    -> hist_weighted_uniform n x w
    | `Bins b, Some w -> hist_weighted_nonuniform b w x
  in
  { bins; counts; weighted_counts; normalised_counts = None; density = None }


let histogram_sorted (bins : [ `N of int | `Bins of float array ]) ?weights x =
  let bins =
    match bins with
  ...
  let (bins, counts), weighted_counts =
    ...
    | Some w -> hist_weighted_sorted bins w x
  in
  { bins; counts; weighted_counts; normalised_counts = None; density = None }
...
let normalise ({ counts; weighted_counts; _ } as h) =
  let nc =
    ...
    | Some wcounts ->
      ...
      Array.map (fun wc -> wc /. total) wcounts
  in
...
let normalise_density ({ bins; counts; weighted_counts; _ } as h) =
  let ds =
    ...
    | None         ->
      let total = Array.fold_left ( + ) 0 counts |> float_of_int in
      Array.mapi (fun i c -> float_of_int c /. (bins.(i + 1) -. bins.(i)) /. total) counts
    | Some wcounts ->
      let total = Array.fold_left ( +. ) 0. wcounts in
      Array.mapi (fun i wc -> wc /. (bins.(i + 1) -. bins.(i)) /. total) wcounts
  in
...
let interquartile x = third_quartile x -. first_quartile x
