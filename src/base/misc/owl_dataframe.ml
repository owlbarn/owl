...
type t =
  ...
  ; (* column-based table, each column is a time series *)
    mutable head : (string, int) Hashtbl.t
  ; (* head and index of each column, stored in a hash table *)
    mutable used : int
  ; (* sise of the used buffer space *)
    mutable size : int (* size of the allocated buffer *)
  }

let unpack_bool = function
  | Bool x -> x
  | _      -> raise Owl_exception.NOT_SUPPORTED
...
let unpack_bool_series = function
  | Bool_Series x -> x
  | _             -> raise Owl_exception.NOT_SUPPORTED


let unpack_int_series = function
  | Int_Series x -> x
  | _            -> raise Owl_exception.NOT_SUPPORTED


let unpack_float_series = function
  | Float_Series x -> x
  | _              -> raise Owl_exception.NOT_SUPPORTED


let unpack_string_series = function
  | String_Series x -> x
  | _               -> raise Owl_exception.NOT_SUPPORTED
...
let make ?data head_names =
  ...
  let size = if col_num = 0 then 0 else length_series data.(0) in
  let used = size in
...
let concat_horizontal x y =
  ...
  Hashtbl.iter (fun k v -> Hashtbl.add head k (v + col_num_x)) y.head;
...
let filteri_row f x =
  ...
  let data = Array.map (fun _ -> Any_Series) x.data in
  let y = { data; head; used; size } in
  iteri_row (fun i row -> if f i row = true then append_row y row) x;
  y
...
let get_slice slice x =
  ...
  let slice =
    Array.map
      (function
        | R_ s -> s
        | _    -> failwith "get_slice: unsupported")
      _tmp0
  in
  let name = Owl_utils_array.get_slice slice.(1) (get_heads x) in
  let data =
    Array.map (head_to_id x) name |> get_cols x |> Array.map (slice_series slice.(0))
  in
...
(* TODO *)
let _set_slice _x = raise (Owl_exception.NOT_IMPLEMENTED "owl_dataframe._set_slice")
...
let get_slice_by_name slice x =
  ...
  let row_slice =
    (function
      ...
      refmt.(0)
  in
  let col_slice = if Array.length col_slice = 0 then get_heads x else col_slice in
...
let sort ?(inc = true) x head =
  ...
  Array.iter (fun i -> get_row x i |> append_row y) indices;
  y
...
let ( .?()<- ) x f g = filter_map_row (fun r -> if f r = true then Some (g r) else None) x
