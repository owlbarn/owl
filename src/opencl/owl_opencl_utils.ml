...
let calc_opt_chunk num_pu total_size =
  ...
  let chunk_size =
    float_of_int total_size /. float_of_int tmp_chunk |> ceil |> int_of_float
  in
  let num_chunk =
    float_of_int total_size /. float_of_int chunk_size |> ceil |> int_of_float
  in
