...
and trace =
  ...
  ; mutable outval : t array
  ; (* output value, not Trace *)
...
let pack_input = function
  ...
  | F32 x   ->
    ...
    let x_mem =
      Owl_opencl_base.Buffer.create_bigarray
        ~flags:[ Owl_opencl_generated.cl_MEM_USE_HOST_PTR ]
        ctx
        x
    in
    { op = Noop ""
...
let pack_arr : type a b. (a, b) Owl_dense_ndarray_generic.t -> t =
 fun x ->
...
let get_input_event x =
  x.input |> Array.fold_left (fun a i -> Array.append a i.events) [||] |> Array.to_list
...
let mapn_arr_eval param fun_name x =
  ...
  Array.iteri
    (fun i (_, _, a_ptr) -> Owl_opencl_base.Kernel.set_arg kernel i sizeof_cl_mem a_ptr)
    args;
  let event = Owl_opencl_base.Kernel.enqueue_ndrange ~wait_for cmdq kernel 1 [ _size ] in
  x.outval <- [| b_val |];
  x.outmem <- [| b_mem |];
...
let _reduce_eval param fun_name wait_for num_groups group_size a_val a_ptr =
  ...
  let event =
    Owl_opencl_base.Kernel.enqueue_ndrange
      ~wait_for
      ~local_work_size
      cmdq
      kernel
...
let reduce_eval param fun_name x =
  ...
  let event, b_val, _, b_ptr =
    _reduce_eval
      ...
      a_ptr
  in
  let event, b_val, b_mem, _ =
    _reduce_eval param fun_name [ event ] 1 group_size b_val b_ptr
  in
  x.outval <- [| F32 b_val |];
  x.outmem <- [| b_mem |];
  x.events <- [| event |]
