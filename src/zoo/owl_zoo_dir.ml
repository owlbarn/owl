...
and _dir_zoo_ocaml gid vid added =
  ...
  |> List.iter (fun l ->
         ...
         let f'_str =
           replace
             "extend_zoo_path"
             (Printf.sprintf "extend_zoo_path ~gid:\"%s\" ~vid:\"%s\"" gid vid)
             f_str
         in
         let gist = gid ^ "?vid=" ^ vid in
         let f'_str =
           replace
             "load_file[ \t]+\\([0-9a-zA-Z'._\"]+\\)"
             (Printf.sprintf "load_file ~gist:\"%s\" \\1" gist)
             f'_str
         in
         Owl_io.write_file f' f'_str;
         _extract_zoo_gist f' added;
         let input = Toploop.File f' in
         Toploop.mod_use_input Format.std_formatter input |> ignore)
...
and process_dir_zoo ?added gist =
  ...
  in
  if Hashtbl.mem added gist' = false
  then (
