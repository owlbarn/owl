...
type cblas_layout =
  | CblasRowMajor
  | CblasColMajor (** Layout type, Row-major or Column-major. *)

type cblas_transpose =
  | CblasNoTrans
  | CblasTrans
  | CblasConjTrans (** Transpose type, no transpose, transpose, or conjugate transpose. *)

type cblas_uplo =
  | CblasUpper
  | CblasLower (** Upper or lower triangular matrix. *)

type cblas_diag =
  | CblasNonUnit
  | CblasUnit (** Diag type, unit triangular. *)
