...
let quantile x =
  ...
  fun p ->
    if p < 0. || p > 1.
    then raise (Invalid_argument "Owl_stats.quantile: expected float between 0 and 1")
    else Owl_stats_extend.quantile x p
...
let t_test_paired ?(alpha = 0.05) ?(side = BothSide) x y =
  ...
  let _ = if nx <> ny then failwith "the sizes of two samples does not equal." in
...
let smirnov n e =
  ...
  let rec helper sum v c =
    ...
    let sum' =
      sum +. (c *. (evn ** float_of_int (v - 1)) *. ((1. -. evn) ** float_of_int (n - v)))
    in
    let c' = c *. float_of_int (n - v) /. float_of_int (v + 1) in
    if v <= nn then helper sum' (v + 1) c' else sum
  in
  let helper2 () =
    ...
    let rec helper3 sum v =
      ...
      let omevn = 1. -. evn in
      let t =
        lgamnp1 -. lngamma (1. +. float_of_int v) -. lngamma (1. +. float_of_int (n + v))
      in
      let sum' = sum +. exp t in
      if v <= nn
      then
        if abs_float omevn > 0. && t > ~-.maxlog
        then helper3 sum' (v + 1)
        else helper3 sum (v + 1)
      else sum
    in
    helper3 0. 0
  in
...
let kolmogorov y =
  ...
  let rec helper sign sum r =
    ...
    let r' = r +. 1. in
    let sign' = ~-.sign in
    if t = 0.0 || t /. sum' <= 1.1e-16 then sum' else helper sign' sum' r'
  in
  if y < 1.1e-16 then 1.0 else 2. *. helper 1. 0. 1.
...
let ks_test ?(alpha = 0.05) x f =
  ...
  if n = 0
  then raise Owl_exception.EMPTY_ARRAY
...
let rec uniques l =
  ...
  | []             -> []
  | [ x ]          -> [ x ]
  | x1 :: x2 :: xs -> if x1 = x2 then uniques (x2 :: xs) else x1 :: uniques (x2 :: xs)
...
let empCdf domain samples =
  ...
  in
  let rec aggregate accum domain samples =
    match domain with
    | []      -> []
    | x :: xs ->
...
let fisher_test ?(alpha = 0.05) ?(side = BothSide) a b c d =
  let cdf ?(max_prob = 1.) k n1 n2 t =
    ...
    Owl_utils.range_fold
      left
      right
      ~f:(fun acc x ->
        let p = hypergeometric_pdf ~good:n1 ~bad:n2 ~sample:t x in
        if p < max_prob || abs_float (p -. max_prob) < eps then acc +. p else acc)
      ~init:0.0
  in
  (* let n = a + b + c + d in *)
  let prob = hypergeometric_pdf ~good:(a + b) ~bad:(c + d) ~sample:(a + c) a in
  let oddsratio =
    float_of_int a *. float_of_int d /. (float_of_int b *. float_of_int c)
  in
  let p =
    match side with
...
let tiecorrect rankvals =
  ...
  let numerator =
    Array.fold_left
      ( + )
      0
      (Array.of_list (List.map (fun (_x, y) -> (y * y * y) - y) counts))
  in
  match size with
  | 0.0 -> 1.0
  | 1.0 -> 1.0
...
let mannwhitneyu ?(alpha = 0.05) ?(side = BothSide) x y =
  let rec exact_a u n m =
    ...
    then float_of_int (Owl_maths.combination (int_of_float n) (int_of_float m))
    else if m = 1. || n -. m = 1.
    then u +. 1.
  ...
  let exact _v =
    ...
    let p =
      ...
      let z =
        exact_a bigu (n1 +. n2) k
        /. float_of_int (Owl_maths.combination (int_of_float n) (int_of_float k))
      in
      match side with
      | BothSide -> 2. *. z
      | _        -> z
    ...
    make_hypothesis h p u2
  in
  if max ranked = n1 +. n2 && max [| n1; n2 |] < 10. then exact 1 else asymptotic 1
...
let wilcoxon ?(alpha = 0.05) ?(side = BothSide) x y =
  ...
  let exact v =
    let rec f w n =
      if w = n *. (n +. 1.) /. 2. || (w = 0. && n >= 0.)
      then 1.
      else if (w < 0. && n > 0.) || (w > 0. && n = 0.) || n < 0.
      then 0.
    ...
    let p =
      ...
      then 0.
        ...
        Array.fold_left
          ...
          0.
          (Owl_utils.Array.map
             (fun i -> f (float_of_int i) n1)
             (Owl_utils.Array.range 0 (int_of_float v)))
    in
    match side with
    | BothSide  -> 2. *. p /. (2. ** n1)
    | RightSide -> 1. -. (p /. (2. ** n1))
    | LeftSide  -> p /. (2. ** n1)
  in
  let p = if Array.length d = Array.length x && n < 10. then exact t else asymptotic 1 in
  let h = alpha > p in
  make_hypothesis h p t
...
let runs_test ?(alpha = 0.05) ?(side = BothSide) ?v x =
  ...
  let r0 = ref 1. in
  let _ =
    for i = 0 to Array.length !z - 2 do
...
let metropolis_hastings f p n =
  let stepsize = 0.1 in
  (* be careful about step size, try 0.01 *)
  let a, b = 1000, 10 in
  ...
  for i = 0 to a + (b * n) - 1 do
    ...
    let y, y' = f p, f p' in
    let p' =
      if y' >= y then p' else if std_uniform_rvs () < y' /. y then p' else Array.copy p
    in
    Array.iteri (fun i x -> p.(i) <- x) p';
    if i >= a && i mod b = 0 then s.((i - a) / b) <- Array.copy p
  done;
  s
