  ...
  for i = 0 to m.n_d - 1 do
    ...
    for j = 0 to dlen - 1 do
      ...
      for k = 0 to m.n_k - 1 do
        wsum
          := !wsum
             +. ((MD.get m.t_dk i k +. m.alpha_k)
                *. (MS.get m.t_wk w k +. m.beta)
                /. (MD.get m.t__k 0 k +. m.beta_v))
      done;
...
let show_info m i t =
  let s =
    match i mod 1 = 0 with
    | true  -> Printf.sprintf "likelihood:%.3f" (likelihood m)
    | false -> ""
  in
  Owl_log.info
    "iter#%i t(s):%.1f t_dk:%.3f t_wk:%.3f %s"
    i
    t
    (MD.density m.t_dk)
    (MS.density m.t_wk)
    ...
    Array.iteri
      (fun i w ->
        ...
        for j = 0 to m.n_k - 1 do
          x
            := !x
               +. ((MD.get m.t_dk d j +. m.alpha_k)
                  *. (MS.get m.t_wk w j +. m.beta)
                  /. (MD.get m.t__k 0 j +. m.beta_v));
          MD.set p 0 j !x
        done;
        (* draw a sample *)
...
module SparseLDA = struct
  ...
  let exclude_token_sparse m w d k ~s ~r ~q =
    ...
    !q.(k) <- (m.alpha_k +. MD.get m.t_dk d k) /. (m.beta_v +. !t__klocal);
        ...
        then (
          ...
          Hashtbl.iter
            (fun key _ ->
              if !qlocal < !u
              then (
                qlocal := !qlocal +. (!q.(key) *. MS.get m.t_wk w key);
                k := key))
            !q_non_zero);
