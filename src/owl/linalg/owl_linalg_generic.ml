...
let select_ev keyword ev =
  ...
  let _ =
    ...
    | `RHP ->
      ...
      M.iteri_2d (fun i j a -> if _op a >= 0. then M.set s i j 1l) ev
    | `UDI ->
      let _op a = Owl_base_dense_common.(_abs_elt k a |> _re_elt k) in
...
let det x =
  ...
  for i = 0 to m - 1 do
    ...
    if M.get ipiv 0 i <> Int32.of_int (i + 1) then c := !c + 1
  done;
...
let _get_qr_q : type a b. (a, b) kind -> (a, b) t -> (a, b) t -> (a, b) t =
 fun k a tau ->
  match k with
...
let _get_lq_q : type a b. (a, b) kind -> (a, b) t -> (a, b) t -> (a, b) t =
 fun k a tau ->
  match k with
...
let lq ?(thin = true) x =
  ...
  let l =
    match thin with
    | true  -> if m < n then M.get_slice [ []; [ 0; minmn - 1 ] ] (M.tril a) else M.tril a
    | false -> M.tril a
  in
  let a =
    match thin with
    | true  -> a
    | false -> if m >= n then a else M.resize ~head:true a [| n; n |]
  in
...
let _magic_complex : type a b c d. (c, d) kind -> (a, b) t -> (a, b) t -> (c, d) t =
 fun otyp re im ->
...
let schur : type a b c d. otyp:(c, d) kind -> (a, b) t -> (a, b) t * (a, b) t * (c, d) t =
 fun ~otyp x ->
  let m, n = M.shape x in
  Owl_exception.(check (m = n) (NOT_SQUARE [| m; n |]));
  let x = M.copy x in
...
let qzvals : type a b c d. otyp:(c, d) kind -> (a, b) t -> (a, b) t -> (c, d) t =
 fun ~otyp x y ->
  let m, n = M.shape x in
  Owl_exception.(check (m = n) (NOT_SQUARE [| m; n |]));
  let u, v = M.shape y in
...
let eig
    : type a b c d.
      ?permute:bool -> ?scale:bool -> otyp:(a, b) kind -> (c, d) t -> (a, b) t * (a, b) t
  =
 fun ?(permute = true) ?(scale = true) ~otyp x ->
  let x = M.copy x in
  let balanc =
    match permute, scale with
    | true, true   -> 'B'
    | true, false  -> 'P'
    | false, true  -> 'S'
    | false, false -> 'N'
      ...
      then
        ...
        for k = 0 to n - 1 do
          ...
          M.set
            ...
            k
            (!j + 1)
            Complex.{ re = M.get vr k !j; im = 0. -. M.get vr k (!j + 1) }
        done;
        j := !j + 1);
      j := !j + 1
    done
...
let eigvals
    : type a b c d.
      ?permute:bool -> ?scale:bool -> otyp:(a, b) kind -> (c, d) t -> (a, b) t
  =
 fun ?(permute = true) ?(scale = true) ~otyp x ->
  let x = M.copy x in
  let balanc =
...
let _get_hess_q : type a b. (a, b) kind -> int -> int -> (a, b) t -> (a, b) t -> (a, b) t =
 fun k ilo ihi a tau ->
  match k with
...
let _minmax_real : type a b. (a, b) kind -> (a, b) t -> float * float =
 fun _k v ->
  match M.kind v with
  | Float32   -> M.minmax' v
  | Float64   -> M.minmax' v
...
let _abs : type a b c. (a, b) kind -> (a, b) t -> (float, c) t =
 fun k x ->
  match k with
...
let norm ?(p = 2.) x =
  let k = M.kind x in
  if p = 1.
  then x |> _abs k |> M.sum_rows |> M.max'
  else if p = -1.
  then x |> _abs k |> M.sum_rows |> M.min'
  else if p = 2.
  then x |> svdvals |> _minmax_real k |> snd
  else if p = -2.
  then x |> svdvals |> _minmax_real k |> fst
  else if p = infinity
  then x |> _abs k |> M.sum_cols |> M.max'
  else if p = neg_infinity
  then x |> _abs k |> M.sum_cols |> M.min'
...
let vecnorm ?(p = 2.) x =
  let k = M.kind x in
  if p = 1.
  then M.l1norm' x |> Owl_base_dense_common._re_elt k
  else if p = 2.
  then M.l2norm' x |> Owl_base_dense_common._re_elt k
  else (
    let v = M.flatten x |> M.abs in
    if p = infinity
    then M.max' v |> Owl_base_dense_common._re_elt k
    else if p = neg_infinity
    then M.min' v |> Owl_base_dense_common._re_elt k
    else (
...
let _get_trans_code : type a b. (a, b) kind -> char = function
  | Float32   -> 'T'
  | Float64   -> 'T'
  | Complex32 -> 'C'
  | Complex64 -> 'C'
...
let triangular_solve
 ...
 fun ~upper ?(trans = false) a b ->
  ...
  let transa =
    ...
    else Owl_cblas_basic.CblasNoTrans
  in
...
let linreg x y =
  ...
  let error () =
    let s =
      Printf.sprintf
        "x length is %i, and y length is %i. However, they must be the same."
        nx
        ny
    in
...
let care ?(diag_r = false) a b q r =
  ...
  in
  let z = M.(concat_vh [| [| a; neg g |]; [| neg q; neg (transpose a) |] |]) in
...
let dare a b q r =
  ...
  let select = M.(zeros int32 (row_num wr) (col_num wr)) in
  M.iter2i_2d
    (fun i j re im -> if Complex.(norm { re; im }) <= 1. then M.set select i j 1l)
    wr
    wi;
  ignore (Owl_lapacke.trsen ~job:'V' ~compq:'V' ~select ~t ~q:u);
  let m, n = M.shape u in
...
let expm_eig : type a b c d. otyp:(c, d) kind -> (a, b) t -> (c, d) t =
 fun ~otyp x ->
  let m, n = M.shape x in
  Owl_exception.(check (m = n) (NOT_SQUARE [| m; n |]));
...
let expm x =
  ...
  then M.exp x
    ...
    if norm_x <= 2.097847961257068
    then (
      let c =
        Array.map
          (Owl_base_dense_common._float_typ_elt (M.kind x))
          (if norm_x > 0.9504178996162932
          then
            [| 17643225600.
             ; 8821612800.
             ; 2075673600.
             ...
             ; 1.
            |]
          else if norm_x > 0.2539398330063230
          then [| 17297280.; 8648640.; 1995840.; 277200.; 25200.; 1512.; 56.; 1. |]
          else if norm_x > 0.01495585217958292
          then [| 30240.; 15120.; 3360.; 420.; 30.; 1. |]
          else [| 120.; 60.; 12.; 1. |])
      in
      let x2 = M.dot x x in
      ...
      let x =
        if s > 0.
        then Owl_base_dense_common._float_typ_elt (M.kind x) (2. ** t) |> M.div_scalar x
        else x
      in
      let c =
        Array.map
...
let _sinm : type a b. (a, b) kind -> (a, b) t -> (a, b) t =
 fun k x ->
  match k with
  | Float32   ->
...
let _cosm : type a b. (a, b) kind -> (a, b) t -> (a, b) t =
 fun k x ->
  match k with
  | Float32   ->
    let a = Complex.{ re = 0.; im = 1. } in
...
let _sincosm : type a b. (a, b) kind -> (a, b) t -> (a, b) t * (a, b) t =
 fun k x ->
  match k with
  | Float32   ->
    let a = Complex.{ re = 0.; im = 1. } in
