...
module Make (A : InpureSig) (B : AttrSig) = struct
  ...
  let node ?(name="") ?(state=Invalid) ?(value=[||]) ?(attr=None) op =
    Owl_graph.node ~name ~prev:[||] ~next:[||] { op; state; value; attr }
  ...
  let copy x =
    ...
    iter_ancestors (fun n ->
      ...
      let a' = match a.op with
        | Var   -> { op = a.op; state = a.state; value = a.value; attr = a.attr }
        | Const -> { op = a.op; state = a.state; value = a.value; attr = a.attr }
        | _     -> { op = a.op; state = Invalid; value = [||]; attr = a.attr }
      in
      set_attr n a';
    ) y;
    y
