...
let zeros ?(density = 0.01) k m n = { m; n; k; d = _eigen_create density k m n }

let eye k m = { m; n = m; k; d = _eigen_eye k m }
...
let insert x i j a = if a <> Owl_const.zero x.k then _eigen_insert x.d i j a
...
let row x i = { m = 1; n = x.n; k = x.k; d = _eigen_row x.d i }

let col x j = { m = x.m; n = 1; k = x.k; d = _eigen_col x.d j }
...
let filteri f x =
  ...
  iteri (fun i j y -> if f i j y then Owl_utils.Stack.push s (i, j)) x;
...
let filteri_nz f x =
  let s = Owl_utils.Stack.make () in
  iteri_nz (fun i j y -> if f i j y then Owl_utils.Stack.push s (i, j)) x;
  Owl_utils.Stack.to_array s
...
let mapi_rows_nz f x =
  let a = _disassemble_rows x in
  let s = Owl_utils.Stack.make () in
  Array.iteri (fun i y -> if nnz y != 0 then Owl_utils.Stack.push s (f i y)) a;
  Owl_utils.Stack.to_array s
...
let add x y = { m = x.m; n = x.n; k = x.k; d = _eigen_add x.d y.d }

let sub x y = { m = x.m; n = x.n; k = x.k; d = _eigen_sub x.d y.d }

let mul x y = { m = x.m; n = x.n; k = x.k; d = _eigen_mul x.d y.d }

let div x y = { m = x.m; n = x.n; k = x.k; d = _eigen_div x.d y.d }

let dot x y = { m = x.m; n = y.n; k = x.k; d = _eigen_gemm x.d y.d }

let add_scalar x a = { m = x.m; n = x.n; k = x.k; d = _eigen_add_scalar x.d a }

let sub_scalar x a = { m = x.m; n = x.n; k = x.k; d = _eigen_sub_scalar x.d a }

let mul_scalar x a = { m = x.m; n = x.n; k = x.k; d = _eigen_mul_scalar x.d a }

let div_scalar x a = { m = x.m; n = x.n; k = x.k; d = _eigen_div_scalar x.d a }
...
let abs x = { m = x.m; n = x.n; k = x.k; d = _eigen_abs x.d }

let neg x = { m = x.m; n = x.n; k = x.k; d = _eigen_neg x.d }
...
let pp_spmat x =
  ...
  if m < 100 && n < 100 then Owl_dense_matrix_generic.print (to_dense x);
...
let rows x l =
  ...
  Array.iteri (fun i k -> iteri_nz (fun _ j v -> insert y i j v) (row x k)) l;
  y


let cols x l =
  let y = zeros x.k x.m (Array.length l) in
  Array.iteri (fun j k -> iteri_nz (fun i _ v -> insert y i j v) (col x k)) l;
  y
