...
let _eigen_nnz : type a b. (a, b) eigen_mat -> int =
 fun x ->
  ...
  | SPMAT_S x ->
    Eigen.Sparse.S.(
      prune x (Owl_const.zero Float32) 0.;
      nnz x)
  | SPMAT_D x ->
    Eigen.Sparse.D.(
      prune x (Owl_const.zero Float64) 0.;
      nnz x)
  | SPMAT_C x ->
    Eigen.Sparse.C.(
      prune x (Owl_const.zero Complex32) 0.;
      nnz x)
  | SPMAT_Z x ->
    Eigen.Sparse.Z.(
      prune x (Owl_const.zero Complex64) 0.;
      nnz x)
...
let _eigen_innerindexptr
    : type a b. (a, b) eigen_mat -> (int64, int64_elt, c_layout) Array1.t
  =
 fun x ->
  match x with
