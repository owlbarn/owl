...
let magic k n =
  ...
  let _magic_singly_even n =
    ...
    let m3 = m / 2 in
    let xa' = concat_horizontal (get_fancy [R[];R[0;m3-1]] xd) (get_fancy [R[];R[m3;-1]] xa) in
    let xd' = concat_horizontal (get_fancy [R[];R[0;m3-1]] xa) (get_fancy [R[];R[m3;-1]] xd) in
    let xb' =
      if m3 - 1 = 0 then xb
      else concat_horizontal (get_fancy [R[];R[0;m-m3]] xb) (get_fancy [R[];R[1-m3;-1]] xc)
    in
    let xc' =
      if m3 - 1 = 0 then xc
      else concat_horizontal (get_fancy [R[];R[0;m-m3]] xc) (get_fancy [R[];R[1-m3;-1]] xb)
    in
