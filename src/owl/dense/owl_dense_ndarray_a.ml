...
let make_arr shape stride data = { shape; stride; data }
...
let get_index x axis =
  ...
  Array.iteri (fun i a -> Array.iteri (fun j b -> indices.(i).(j) <- b) a) axis;
...
let copy x =
  { shape = Array.copy x.shape; stride = Array.copy x.stride; data = Array.copy x.data }
...
let map f x = make_arr x.shape x.stride (Array.init (numel x) (fun i -> f x.data.(i)))

let mapi f x = make_arr x.shape x.stride (Array.init (numel x) (fun i -> f i x.data.(i)))
...
let map2 f x y =
  assert (x.shape = y.shape);
  make_arr x.shape x.stride (Array.init (numel x) (fun i -> f x.data.(i) y.data.(i)))


let map2i f x y =
  assert (x.shape = y.shape);
  make_arr x.shape x.stride (Array.init (numel x) (fun i -> f i x.data.(i) y.data.(i)))
...
let repeat x reps =
  ...
  if Array.for_all (fun x -> x = 1) reps = true
  then copy x
  else (
    ...
    then (
      ...
      for _i = 0 to block_num.(0) - 1 do
        ...
        if block_sz = 1
        then Array.blit x' !ofsx y' !ofsy_sub slice_x.(hd)
        else
...
let get_fancy axis x =
  ...
  let high_dim_list =
    (function
      ...
      axis.(d0 - 1)
  in
  if cb > 1 || s0.(d0 - 1) = 1 || high_dim_list = true
  then (
    ...
    let c, dd =
      ...
      | R_ i -> if i.(2) > 0 then i.(2), i.(0) else i.(2), i.(0) + ((b - 1) * i.(2))
    ...
    let f i =
      ...
      let ofsy = if c > 0 then ref (!ofsy_i * b) else ref (((!ofsy_i + 1) * b) - 1) in
      for _i = 0 to b - 1 do
        y'.(!ofsy) <- x'.(!ofsx);
        ofsx := !ofsx + cx;
        ofsy := !ofsy + cy
...
let index_nd_1d i_nd _stride = Owl_utils.index_nd_1d i_nd _stride
