...
let broadcast_align_shape x0 x1 =
  ...
  let s0 = shape y0 in
  let s1 = shape y1 in
  Array.iter2 (fun a b -> assert (not (a <> 1 && b <> 1 && a <> b))) s0 s1;
...
let gaussian_logpdf ~mu ~sigma x =
  broadcast_op2 (_owl_gaussian_logpdf (kind x)) mu sigma x


let gaussian_cdf ~mu ~sigma x = broadcast_op2 (_owl_gaussian_cdf (kind x)) mu sigma x

let gaussian_logcdf ~mu ~sigma x =
  broadcast_op2 (_owl_gaussian_logcdf (kind x)) mu sigma x
...
let exponential_rvs ~lambda ~n =
  broadcast_op1 (_owl_exponential_rvs (kind lambda)) lambda n


let exponential_pdf ~lambda x =
  broadcast_op2 (_owl_exponential_pdf (kind x)) lambda lambda x


let exponential_logpdf ~lambda x =
  broadcast_op2 (_owl_exponential_logpdf (kind x)) lambda lambda x


let exponential_cdf ~lambda x =
  broadcast_op2 (_owl_exponential_cdf (kind x)) lambda lambda x


let exponential_logcdf ~lambda x =
  broadcast_op2 (_owl_exponential_logcdf (kind x)) lambda lambda x


let exponential_ppf ~lambda x =
  broadcast_op2 (_owl_exponential_ppf (kind x)) lambda lambda x


let exponential_sf ~lambda x =
  broadcast_op2 (_owl_exponential_sf (kind x)) lambda lambda x


let exponential_logsf ~lambda x =
  broadcast_op2 (_owl_exponential_logsf (kind x)) lambda lambda x


let exponential_isf ~lambda x =
...
let gamma_logpdf ~shape ~scale x =
  broadcast_op2 (_owl_gamma_logpdf (kind x)) shape scale x


let gamma_cdf ~shape ~scale x = broadcast_op2 (_owl_gamma_cdf (kind x)) shape scale x

let gamma_logcdf ~shape ~scale x =
  broadcast_op2 (_owl_gamma_logcdf (kind x)) shape scale x
...
let lomax_logpdf ~shape ~scale x =
  broadcast_op2 (_owl_lomax_logpdf (kind x)) shape scale x


let lomax_cdf ~shape ~scale x = broadcast_op2 (_owl_lomax_cdf (kind x)) shape scale x

let lomax_logcdf ~shape ~scale x =
  broadcast_op2 (_owl_lomax_logcdf (kind x)) shape scale x
...
let weibull_rvs ~shape ~scale ~n =
  broadcast_op0 (_owl_weibull_rvs (kind shape)) shape scale n


let weibull_pdf ~shape ~scale x = broadcast_op2 (_owl_weibull_pdf (kind x)) shape scale x

let weibull_logpdf ~shape ~scale x =
  broadcast_op2 (_owl_weibull_logpdf (kind x)) shape scale x


let weibull_cdf ~shape ~scale x = broadcast_op2 (_owl_weibull_cdf (kind x)) shape scale x

let weibull_logcdf ~shape ~scale x =
  broadcast_op2 (_owl_weibull_logcdf (kind x)) shape scale x
...
let weibull_logsf ~shape ~scale x =
  broadcast_op2 (_owl_weibull_logsf (kind x)) shape scale x
...
let laplace_logpdf ~loc ~scale x =
  broadcast_op2 (_owl_laplace_logpdf (kind x)) loc scale x


let laplace_cdf ~loc ~scale x = broadcast_op2 (_owl_laplace_cdf (kind x)) loc scale x

let laplace_logcdf ~loc ~scale x =
  broadcast_op2 (_owl_laplace_logcdf (kind x)) loc scale x
...
let logistic_logpdf ~loc ~scale x =
  broadcast_op2 (_owl_logistic_logpdf (kind x)) loc scale x


let logistic_cdf ~loc ~scale x = broadcast_op2 (_owl_logistic_cdf (kind x)) loc scale x

let logistic_logcdf ~loc ~scale x =
  broadcast_op2 (_owl_logistic_logcdf (kind x)) loc scale x
...
let logistic_logsf ~loc ~scale x =
  broadcast_op2 (_owl_logistic_logsf (kind x)) loc scale x
...
let lognormal_logpdf ~mu ~sigma x =
  broadcast_op2 (_owl_lognormal_logpdf (kind x)) mu sigma x


let lognormal_cdf ~mu ~sigma x = broadcast_op2 (_owl_lognormal_cdf (kind x)) mu sigma x

let lognormal_logcdf ~mu ~sigma x =
  broadcast_op2 (_owl_lognormal_logcdf (kind x)) mu sigma x
...
let lognormal_logsf ~mu ~sigma x =
  broadcast_op2 (_owl_lognormal_logsf (kind x)) mu sigma x
