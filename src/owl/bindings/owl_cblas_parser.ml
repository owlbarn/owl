...
let print_help () =
  print_endline "Usage: EXE cblas.h ctypes_output_file binding_output_file"
...
let convert_typ_to_ctypes fun_blas typ_name =
  ...
  | "void*"           ->
    ...
    if prefix8 = "cblas_sc"
    then "ptr complex32"
    else if prefix8 = "cblas_dz"
    then "ptr complex64"
    else if prefix8 = "cblas_ic"
    then "ptr complex32"
    else if prefix8 = "cblas_iz"
    then "ptr complex64"
    else (
      match prefix7 with
...
let parse_cblas_binding fname =
  ...
  (try
     while true do
       ...
       let _typ_name =
         Str.matched_group 3 s |> Str.global_replace (Str.regexp "[ \n]+") " "
       in
       let _fun_caml =
         _fun_blas
       ...
       if _fun_blas <> "cblas_xerbla"
       then funs := Array.append !funs [| _fun_caml, _fun_blas, _typ_name, _fun_rval |]
     done
   with
  | exn -> ());
  !funs
...
let convert_to_ctypes_fun funs =
  Array.mapi
    (fun i (_fun_caml, _fun_blas, _typ_s, _fun_rval) ->
      ...
      let fun_s =
        Printf.sprintf "let cblas_%s = foreign \"%s\" (%s)\n" _fun_caml _fun_blas args_s
      in
      fun_s)
    funs
...
let convert_cblas_header_to_ctypes fname funs =
  ...
  Fun.protect
    (fun () ->
      ...
      Array.iter
        (fun fun_s -> Printf.fprintf h_ml "  %s\n" fun_s)
...
let convert_cblas_header_to_extern fname funs =
  ...
  Fun.protect
    (fun () ->
      ...
      Array.iter
        (fun (fun_caml, fun_stub_s, args, fun_rval) ->
          Printf.fprintf h_ml "%s\n" fun_stub_s)
