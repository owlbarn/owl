...
let print_help () =
  print_endline "Usage: EXE lapacke.h ctypes_output_file binding_output_file"
...
let parse_lapacke_header fname =
  ...
  (try
     while true do
       ...
       if is_work_fun _s = false
       then (
         let regex = Str.regexp "[ \n]+" in
         let _s = Str.global_replace regex " " _s in
         if is_in_funlist t _s = true then funs := Array.append !funs [| _s |]);
...
let convert_to_ctypes_fun funs =
  ...
  Array.map
    (fun s ->
      ...
      let fun_s =
        Printf.sprintf "  let %s = foreign \"LAPACKE_%s\" %s\n" _fun_name _fun_name args
      in
      fun_s)
    funs
    ...
    Array.fold_left
      (fun a arg ->
        ...
        let s =
          ...
          then Printf.sprintf "(CI.cptr %s)" s
