...
let kron a b =
  (* Kronecker product according to https://en.wikipedia.org/wiki/Kronecker_product *)
  let nra, nca = Mat.shape a in
  let nrb, ncb = Mat.shape b in
  let nrk, nck = ((nra*nrb), (nca*ncb)) in
  let k = Mat.create nrk nck 0.0
  in
  for i = 1 to nrk do
    for j = 1 to nck do
(* The index generation is 1-indexed, which is why we subtract 1 in the addressing *)
