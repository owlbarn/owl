...
module Model = struct
  ...
  let t_dk = MS.zeros 0 0        (* document-topic table: num of tokens assigned to each topic in each doc *)
  let t_wk = MS.zeros 0 0        (* word-topic table: num of tokens assigned to each topic for each word *)
  let t__k = MS.zeros 1 n_k      (* number of tokens assigned to a topic: k = sum_w t_wk = sum_d t_dk *)
  let t__z = MS.zeros 0 0        (* table of topic assignment of each token in each document *)

  let data = MS.zeros 0 0  (* training data *)
  let n_iter = 1_000       (* number of iterations *)

  let init () = ()

  let include_topic w d k =
    MS.(set t__k 0 k (get t__k 0 k +. 1.));
    MS.(set t_wk w k (get t_wk w k +. 1.));
    MS.(set t_dk d k (get t_dk d k +. 1.))

  let exclude_topic w d k =
    MS.(set t__k 0 k (get t__k 0 k -. 1.));
    MS.(set t_wk w k (get t_wk w k -. 1.));
    MS.(set t_dk d k (get t_dk d k -. 1.))
