...
module Cubic = struct
  ...
  let f t x =
    ...
    let u2 = u *. u in
    let u3 = u2 *. u in
    (t.poly.(0) *. u3) +. (t.poly.(1) *. u2) +. (t.poly.(2) *. u) +. t.poly.(3)
  ...
  let df t x =
    let u = (x -. t.lx) /. t.dx in
    let u2 = u *. u in
    ((t.poly.(0) *. u2 *. 3.) +. (t.poly.(1) *. u *. 2.) +. t.poly.(2)) /. t.dx
...
module CdfTree = struct
  ...
  let rec display ?(prefix = "") ?(dirn = 0) t =
    match t with
  ...
  let cdf_x_of_p ?(min_x = -1.E4) ?(max_x = 1.E4) cdf p =
    let rec find_x_of_p lx rx =
      let dx = rx -. lx in
      let mx = (lx +. rx) /. 2. in
      let mp = cdf mx in
      if dx < 1E-8 then mx else if mp > p then find_x_of_p lx mx else find_x_of_p mx rx
    in
  ...
  let rec value t x =
    match t with
    | Leaf n               -> n
    | Balance (nx, nl, nr) -> if x < nx then value nl x else value nr x


  let create_from_rv rv logn = create (cdf_x_of_p rv) logn
end
...
let _ =
  ...
  List.iter
    (fun x -> Printf.printf "bucket of %f: %d\n" x (CdfTree.value tree x))
    [ -1.; -0.5; 0.; 0.5; 1. ]
...
module Distribution = struct
  ...
  type t =
    { name : string
    ; num_buckets : int
    ; cdf_x_array : float array
    ; (* i -> x: cdf(x) = i/num_buckets *)
      pdf_x_array : float array
    ; (* i -> pdf(x : cdf(x)=i/num_buckets ) *)
  ...
  let create name dut_cdf dut_pdf cdf_x_array pdf_x_array =
    ...
    let cubics =
      let rec add_cubic acc i =
        (* build acc up in forward order *)
        if i >= 0
        then (
  ...
  let search_for_x t x =
    ...
    in
    let li, ri =
      if x <= t.cdf_x_array.(0)
      then 0, 1
      else if x >= t.cdf_x_array.(t.num_buckets)
      then t.num_buckets - 1, t.num_buckets
...
module RandomTest = struct
  ...
  (*t make_hypothesis *)
  let make_hypothesis reject p_value score : M.hypothesis = { reject; p_value; score }
  ...
  (*t assert_hypothesis *)
  let assert_hypothesis ?(should_reject = false) rv_name test_hypothesis =
    ...
    Alcotest.(check bool)
      (sfmt "Statistical test '%s' for random variable '%s'" name rv_name)
      should_reject
      hypothesis.reject;
    ()
  ...
  let match_distribution ?(significance = 0.01) (dist : Distribution.t) =
    let diff i =
      let x = float i /. 32. in
    ...
    in
    let p = sum_diff2 0. 32 /. 32. in
    (* NOT a probability, but a closeness... *)
  ...
  let do_fft ?(significance = 0.01) ?(n = 1024) f iter =
    let x = C.zeros 1 n in
    let blah acc i b =
      if i < n then C.set x 0 i { Complex.re = f b; im = 0. };
      (* -1 or +1 *)
      acc
    in
    ...
    let rec count_above_threshold i acc =
      ...
      then acc
        ...
        let new_acc = if Complex.norm2 c < t2 then acc + 1 else acc in
    ...
    let p = Maths.erfc x in
    (* probability of seeing x given the distribution actually is mean 0, sd=1/sqrt(2) *)
    let hypothesis =
      make_hypothesis (p < significance) p (float n_over /. float (n / 2))
    in
    hypothesis
...
module BinaryTest = struct
  ...
  let frequency ?(significance = 0.01) ?(p = 0.5) iter =
    ...
    let n, sum = iter inc_if_true_else_dec 0 in
    (* if p=0.5  then sum should have mean 0, sd=sqrt(n) *)
    let x = float sum -. (((2. *. p) -. 1.) *. float n) in
    (* x always should have mean 0, sd=sqrt(n*4pq) *)
    let x = x /. sqrt (8. *. p *. q *. float n) in
    (* x always should have mean 0, sd=1/sqrt(2) *)
    let x = abs_float x in
    let p = Maths.erfc x in
    (* probability of seeing x given the distribution actually is mean 0, sd=1/sqrt(2) *)
  ...
  let block_frequency ?(significance = 0.01) ?(p = 0.5) m iter =
    ...
    make_test_hypothesis
      (sfmt "Block frequency of %d run lengths of size %d" n m)
      "Chi^2"
      hypothesis
  ...
  let occurrence_of_patterns ?(significance = 0.01) m iter =
    ...
    let rec count_ones_in_n acc i v =
      if i == 0 then acc else count_ones_in_n (acc + (v land 1)) (i - 1) (v lsr 1)
    in
    let count_observations (sum_z, sum_o, m_seen, pat) _ b =
      ...
      else sum_z + 1 - v, sum_o + v, m_seen + 1, new_pat
    in
    ...
    let set_exp_counts i =
      ...
      (prob_o ** float num_o) *. (prob_z ** float num_z) *. float (n - m + 1)
    in
    ...
    let hypothesis = make_hypothesis (p < significance) p chi2 in
    make_test_hypothesis
      (sfmt "Occurrences of %d-bit patterns in run length of size %d" m n)
      "Chi^2"
      hypothesis
  ...
  let occurrences_of_patterns ?(significance = 0.01) n m iter =
    ...
    let num_passing =
      List.fold_left (fun a i -> if is_fail i then a else a +. 1.) 0. run_results
    in
    ...
    make_test_hypothesis
      (sfmt "%d iterations of '%s'" n (List.hd run_results).test_name)
      "Num passing"
      hypothesis


  (*f fft *)
  let fft ?(significance = 0.01) ~n iter =
    let hypothesis = do_fft ~significance ~n (fun b -> if b then 1. else -1.) iter in
...
let test_rough_cdf pydist =
  ...
  for i = n * 2 / 100 to n * 98 / 100 do
    ...
    let err_ppf_2, err_isf_2 =
      (dut_cdf_ppf_p -. p) ** 2., (dut_ppf_p -. dut_isf_o_m_p) ** 2.
    in
  ...
  Printf.printf
    "%s: Avg sq diff in cdf %g pdf %g max cdf %g pdf %g\n"
    name
    avg_diff_cdf_2
    avg_diff_pdf_2
    !max_diff_cdf_2
    !max_diff_pdf_2;
  Printf.printf
    "         Avg err logcdf %g logpdf %g ppf %g isf %g\n"
    avg_err_log_cdf_2
    avg_err_log_pdf_2
    avg_err_ppf_2
    avg_err_isf_2;
  ...
  Alcotest.(check bool)
    (sfmt "Avg sq diff in '%s' cdf<1E-8" name)
    true
    (avg_diff_cdf_2 < 1E-8 *. imprecision);
  Alcotest.(check bool)
    (sfmt "Max sq diff in '%s' cdf<1E-5" name)
    true
    (!max_diff_cdf_2 < 1E-5 *. imprecision);
  Alcotest.(check bool)
    (sfmt "Avg sq diff in '%s' pdf<1E-3" name)
    true
    (avg_diff_pdf_2 < 1E-3 *. imprecision);
  Alcotest.(check bool)
    (sfmt "Max sq diff in '%s' pdf<1E-1" name)
    true
    (!max_diff_pdf_2 < 1E-1 *. imprecision);
  Alcotest.(check bool)
    (sfmt "Avg logcdf error in '%s' <1E-28" name)
    true
    (avg_err_log_cdf_2 < 1E-28);
  Alcotest.(check bool)
    (sfmt "Avg logpdf error in '%s' <1E-28" name)
    true
    (avg_err_log_pdf_2 < 1E-28);
  Alcotest.(check bool)
    (sfmt "Avg ppf error in '%s' <1E-28" name)
    true
    (avg_err_ppf_2 < 1E-28);
  Alcotest.(check bool)
    (sfmt "Avg isf error in '%s' <1E-28" name)
    true
    (avg_err_isf_2 < 1E-28);
  Alcotest.(check bool)
    (sfmt "Avg sf error in '%s' <1E-28" name)
    true
    (avg_err_sf_2 < 1E-28);
  Alcotest.(check bool)
    (sfmt "Avg log sf error in '%s' <1E-28" name)
    true
    (avg_err_log_sf_2 < 1E-28);
  ()
...
(* Distribution of (# of 1s in runs of length 7) is uniform *)
...
(* Distribution of (# of 1s in runs of length 70) is uniform *)

let bin_block_frequency_700 = IIFIOfBool (BinaryTest.block_frequency 700)

(* Distribution of (# of 1s in runs of length 700) is uniform *)

let bin_block_frequency_1_of_3_7 = IIFIOfBool (BinaryTest.block_frequency ~p:one_third 7)

(* Distribution of (# of 1s in runs of length 7) is uniform *)

let bin_block_frequency_1_of_3_70 =
  IIFIOfBool (BinaryTest.block_frequency ~p:one_third 70)


(* Distribution of (# of 1s in runs of length 70) is uniform *)

let bin_block_frequency_1_of_3_700 =
  IIFIOfBool (BinaryTest.block_frequency ~p:one_third 700)
...
(* Distribution of overlapping 2-bit patterns is uniform *)

let bin_patterns_3 = IIIIOfBool (BinaryTest.occurrences_of_patterns 8 3)

(* Distribution of overlapping 3-bit patterns is uniform *)

let bin_patterns_4 = IIIIOfBool (BinaryTest.occurrences_of_patterns 8 4)

(* Distribution of overlapping 4-bit patterns is uniform *)

let bin_patterns_5 = IIIIOfBool (BinaryTest.occurrences_of_patterns 8 5)

(* Distribution of overlapping 5-bit patterns is uniform *)

let bin_patterns_6 = IIIIOfBool (BinaryTest.occurrences_of_patterns 8 6)

(* Distribution of overlapping 6-bit patterns is uniform *)

let bin_patterns_7 = IIIIOfBool (BinaryTest.occurrences_of_patterns 8 7)

(* Distribution of overlapping 7-bit patterns is uniform *)

let bin_patterns_8 = IIIIOfBool (BinaryTest.occurrences_of_patterns 8 8)

(* Distribution of overlapping 8-bit patterns is uniform *)

let bin_patterns_9 = IIIIOfBool (BinaryTest.occurrences_of_patterns 8 9)

(* Distribution of overlapping 9-bit patterns is uniform *)
...
let run_random_tests test_name pretest sampler tests =
  let run_test (test, should_reject, start, length) =
    ...
    let test_result =
      match test with
      | IOfBool t -> t (RandomTest.iterate sampler start length)
      (* Note type of iterator depends on type of t*)
      | IIFIOfBool t -> t (RandomTest.iterate sampler start length)
      (* Note type of iterator depends on type of t*)
      | IIIIOfBool t -> t (RandomTest.iterate sampler start length)
      (* Note type of iterator depends on type of t*)
    in
    RandomTest.assert_hypothesis test_name ~should_reject test_result
  in
...
let test_uniform_ints_0_to_65535 (bit : int) _ =
  ...
  let pretest _ = Owl_stats_prng.sfmt_seed 0 in
  let sampler _ length =
    length, fun _ -> (M.uniform_int_rvs ~a:0 ~b:65535 lsr bit) land 1 == 0
  in
  let tests = tests_for_p_0_5 in
  run_random_tests test_name pretest sampler tests
...
let test_hypergeometric_50_500_100 _ =
  ...
  let pretest _ = Owl_stats_prng.sfmt_seed 2 in
  (* fails in one of the patterns with seed 1 *)
  let sampler _ length =
    length, fun _ -> M.hypergeometric_rvs ~good:100 ~bad:(1000 - 100) ~sample:195 >= 20
  in
  let tests = tests_for_p_0_5 in
  run_random_tests test_name pretest sampler tests
...
let test_hypergeometric_50_500_100_p0_3_left _ =
  ...
  (* fails in one of the patterns with seed 1 *)
  let sampler _ length =
    length, fun _ -> M.hypergeometric_rvs ~good:100 ~bad:(1000 - 100) ~sample:180 >= 20
  in
  let tests = tests_for_p_0_3 in
  run_random_tests test_name pretest sampler tests
...
let test_hypergeometric_50_500_100_p0_3_right _ =
  ...
  (* fails in one of the patterns with seed 1 *)
  let sampler _ length =
    length, fun _ -> M.hypergeometric_rvs ~good:100 ~bad:(1000 - 100) ~sample:212 < 20
  in
  let tests = tests_for_p_0_3 in
  run_random_tests test_name pretest sampler tests
...
(* The tests *)
let test_set =
  [ "true_is_not_random", `Slow, test_true_is_not_random
  ; (* tests the test infrastructure *)
    "alternate_is_not_random", `Slow, test_alternate_is_not_random
  ; (* tests the test infrastructure *)
  ...
  ; ( "hypergeometric_50_500_100_p0_3_right"
    , `Slow
    , test_hypergeometric_50_500_100_p0_3_right )
